...
def clean_errors(log_step=1000, keep_days=7, batch_size=100):
    """
    ...
    errors = 0
    logger.info('Deleting Error (max date = %s, total: %d)', max_date, count)
    stop = False
    iterate = 0
    while not stop:
        ...
        for instance in instances:
            ...
            if iterate and not iterate % log_step:
                logger.info('Deleted %d on %d', done, count)
            try:
    ...
    logger.info('Total Error deleted: %d on %d, keep %d (including %d errors)', done, count, count - done, errors)


def clean_job_model(job_model, log_step=1000, keep_days=7, batch_size=100):
    """
    ...
    logger.info('Deleting %s (max date = %s, total: %d (planned: %d))', model_repr, max_date, count, len(planned))
    kept = 0
    iterate = 0
    while True:
        ...
        for instance in instances:
            iterate += 1
            if iterate and not iterate % log_step:
                logger.info('Deleted %d on %d (kept %d)', done, count, kept)
            try:
    ...
    logger.info('Total %s deleted: %d on %d, keep %d (including %d errors and %d planned)', model_repr, done, count, kept, errors, len(planned))


def clean_all(log_step=1000, keep_days=7, batch_size=100):
    """
    Remove from the Error and all Job models entries older than the given number of days
    """
    clean_errors(log_step=log_step, keep_days=keep_days, batch_size=batch_size)
    for J in JobRegistry:
        clean_job_model(J, log_step=log_step, keep_days=keep_days, batch_size=batch_size)
