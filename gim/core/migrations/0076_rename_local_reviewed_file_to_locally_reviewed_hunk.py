...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Renaming table 'LocalReviewedFile' to 'LocallyReviewedHunk'
        db.rename_table(u'core_localreviewedfile', u'core_locallyreviewedhunk')

        if not db.dry_run:
            orm['contenttypes.contenttype'].objects.filter(
                app_label='core', model='LocalReviewedFile').update(model='LocallyReviewedHunk')

        # Adding unique constraint on 'LocallyReviewedHunk', fields ['repository', 'author', 'path', 'patch_sha']
        db.create_unique(u'core_locallyreviewedhunk', ['repository_id', 'author_id', 'path', 'patch_sha'])


    def backwards(self, orm):
        # Removing unique constraint on 'LocallyReviewedHunk', fields ['repository', 'author', 'path', 'patch_sha']
        db.delete_unique(u'core_locallyreviewedhunk', ['repository_id', 'author_id', 'path', 'patch_sha'])

        # Renaming table 'LocallyReviewedHunk' to 'LocalReviewedFile'
        db.rename_table(u'core_locallyreviewedhunk', u'core_localreviewedfile')

        if not db.dry_run:
            orm['contenttypes.contenttype'].objects.filter(
                app_label='core', model='LocallyReviewedHunk').update(model='LocalReviewedFile')
