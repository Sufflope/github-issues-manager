...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Move assignee from FK to the new M2M
        through_model = orm.Issue._meta.get_field('assignees').rel.through
        entries = []
        for issue_id, assignee_id in orm.Issue.objects.filter(assignee__isnull=False).values_list('id', 'assignee_id'):
            entries.append(through_model(issue_id=issue_id, githubuser_id=assignee_id))
        print('Converting %d assignee FK to M2M' % len(entries))
        through_model.objects.bulk_create(entries, batch_size=500)

    def backwards(self, orm):
        # Move assignee from M2M to FK
        qs = orm.Issue.objects.filter(assignees__isnull=False)
        print('Converting %d assignee M2M to FK' % qs.count())
        for issue in queryset_iterator(qs):
            assignees = list(issue.assignees.values_list('id', flat=True))  # id is the user id
            if assignees:
                issue.assignee_id = assignees[0]
                issue.save(update_fields=['assignee_id'])
    ...
    models = {
        ...
        'core.issue': {
            'Meta': {'unique_together': "(('repository', 'number'),)", 'object_name': 'Issue', 'index_together': "[('repository', 'state'), ('repository', 'assignee'), ('repository', 'milestone'), ('repository', 'milestone', 'state'), ('repository', 'milestone', 'state', 'assignee')]"},
            ...
            'assignees': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'assigned_issues2'", 'symmetrical': 'False', 'to': "orm['core.GithubUser']"}),
