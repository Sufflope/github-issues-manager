...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for pr in queryset_iterator(
                orm.Issue.objects.filter(is_pull_request=True, state='open', last_head_commit__isnull=True)):
            try:
                c = pr.commits.get(sha=pr.head_sha)
            except pr.commits.model.DoesNotExist:
                pass
            else:
                if c != pr.last_head_commit:
                    pr.last_head_commit = c
                    pr.save(update_fields=['last_head_commit'])
                    if c:
                        orm.IssueCommits.objects.get_or_create(
                            issue=pr, commit=c,
                            defaults={'deleted': False, 'pull_request_head_at':  c.committed_at}
                        )

        for pr in queryset_iterator(
                orm.Issue.objects.filter(is_pull_request=True, state='open', last_head_commit__isnull=False)):
            ics = pr.related_commits.filter(
                commit__author=pr.last_head_commit.author, commit__authored_at=pr.last_head_commit.authored_at
            ).select_related('commit').order_by('commit__committed_at')
            if ics:
                for ic in ics:
                    if ic.pull_request_head_at != ic.commit.committed_at:
                        ic.pull_request_head_at=ic.commit.committed_at
                        ic.save(update_fields=['pull_request_head_at'])
                try:
                    status = [ic.commit.last_status for ic in ics if ic.commit.last_status][-1]
                except IndexError:
                    pass
                else:
                    if pr.last_head_status != status:
                        pr.last_head_status = status
                        pr.save(update_fields=['last_head_status'])

    def backwards(self, orm):
        # Nothing to do in backwards mode
        pass
